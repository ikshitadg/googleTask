...
'use strict';
/**
 * @description:for reactive a completed task
 */
...
let validator = joi.object({
  status: joi.string().required().description("active").error(new Error('status is missing')),
  statusCode: joi.number().default(1).required().description("1").error(new Error('statusCode is missing'))
});
